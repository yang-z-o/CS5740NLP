...
def main():
    ...
    kv = k * len(unigram)
    pred_md = []
    emission_unk_0 = (md0_count['unk'] + k) / (cnt_0 + kv)
    emission_unk_1 = (md1_count['unk'] + k) / (cnt_1 + kv)
    ...
    for i in range(len(val_word)):
        ...
        if unigram.get(val_word[i][0], 0) is 0:
            ...
            emission_p0 = (md0_count.get(val_word[i][0], 0) + k) / (cnt_0 + kv)
            emission_p1 = (md1_count.get(val_word[i][0], 0) + k) / (cnt_1 + kv)
        ...
        for j in range(1,n):
            if unigram.get(val_word[i][j], 0) is 0:
                ...
                emission_p0 = (md0_count.get(val_word[i][j], 0) + k) / (cnt_0 + kv)
                emission_p1 = (md1_count.get(val_word[i][j], 0) + k) / (cnt_1 + kv)
