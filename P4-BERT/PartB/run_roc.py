...
# Modifier: Yangmengyuan Zhao (yz2453)
#
# Finetuning the library models for ROCStories.
...
from utils_roc import (convert_examples_to_features, processors)
...
def test(args, model, tokenizer, prefix=""):
    ...
    test_outputs_dirs = (args.output_dir,)

    results = {}
    for test_task, test_output_dir in zip(test_task_names, test_outputs_dirs):
        test_dataset = load_and_cache_examples(args, test_task, tokenizer, test=True)
        ...
        with open('output.csv', 'w') as csvfile:
            ...
            for i in range(len(preds)):
                filewriter.writerow([examples[i].example_id, label_map[preds[i]]])
